...
void CWiiViewDlg::OnBnClickedButtonOpen()
{
	...
	// open
	CFileDialog diaOpenIso(TRUE, L"iso");
...
void CWiiViewDlg::LoadIso()
{
	...
	FILE* IsoFile = _wfopen(csIsoPathName, L"rb");
	...
	{
	case 'R':
		m_edtDiskType.SetWindowText(L"Wii");
		break;
	case 'G':
		m_edtDiskType.SetWindowText(L"GameCube");
		break;
	case 'U':
		m_edtDiskType.SetWindowText(L"Utility");
	    break;
	default:
		m_edtDiskType.SetWindowText(L"Unknown");
	    break;
	}
	...
	{
	case 'E':
		m_edtRegion.SetWindowText(L"USA");
		break;
	case 'P':
		m_edtRegion.SetWindowText(L"PAL");
		break;
	case 'J':
		m_edtRegion.SetWindowText(L"JAP");
	    break;
	default:
		m_edtRegion.SetWindowText(L"Unknown");
	    break;
	}
	...
	m_edtGameTite.SetWindowText(CString(IsoHead.gameTitle));

	CString csPartCount;
	csPartCount.Format(L"%d", toDWORD(PartitionsInfo.partitionsCount));
	m_edtPartCount.SetWindowText(csPartCount);

	for (int i = 0; i < toDWORD(PartitionsInfo.partitionsCount); i++)
	{
		CString csPart;
		csPart.Format(L"Partition %d", i);
...
void CWiiViewDlg::FillTreeView(TFSTItem* fst, DWORD fstSize)
{
	...
	HTREEITEM hRoot = m_tvMain.InsertItem(CString((LPCSTR)"Root"), 0);
	...
	for (i = 1; i < fstItemCount; i++)
	{
		...
		case 0:
			{
				HTREEITEM hItem = m_tvMain.InsertItem(CString((LPCSTR)fileNameTableBase + fileNameOffset), 1, 1, hRoot);
		...
		case 1:
			{
				if (0 == toDWORD(fst[i].FileOffset))
				{
					...
					HTREEITEM hNewItem =  m_tvMain.InsertItem(CString((LPCSTR)fileNameTableBase + fileNameOffset), 0 ,0 ,hRoot);
...
void CWiiViewDlg::AddTreeViewItem(HTREEITEM base, TFSTItem* fst, int begin, int count, char* nameTableBase)
{
	...
	for (i = begin; i< begin + count; i++)
	{
		...
		{
			//DWORD fileNameOffset = (fst[i].FileName[2]) | ((fst[i].FileName[1] << 8) & 0xff00) | ((fst[i].FileName[0] << 16) & 0xff0000);
			HTREEITEM hItem = m_tvMain.InsertItem(CString((LPCSTR)nameTableBase + fileNameOffset), 1, 1, base);
			m_tvMain.SetItemData(hItem, i);
			//i++;
		}
		else
		{
			...
			//DWORD fileNameOffset = (fst[i].FileName[2]) | ((fst[i].FileName[1] << 8) & 0xff00) | ((fst[i].FileName[0] << 16) & 0xff0000);
			HTREEITEM hNewItem = m_tvMain.InsertItem(CString((LPCSTR)nameTableBase + fileNameOffset), 0,  0, base);
			m_tvMain.SetItemData(hNewItem, i);
...
void CWiiViewDlg::OnBnClickedButtonExtract()
{
	...
	if (diaSaveFile.DoModal() == IDOK)
	{
		...
		fp = _wfopen(diaSaveFile.GetPathName(), L"wb");
		//if (fp == 0)
